...
void RefineGravityWithWD(map<double, ImageFrame> &all_image_frame, Vector3d &g, VectorXd &x) // new
{
    ...
    for (int k = 0; k < 4; k++)
    {
        ...
        for (frame_i = all_image_frame.begin(); next(frame_i) != all_image_frame.end(); frame_i++, i++)
        {
            ...
            tmp_A.block<3, 1>(6, 7) = (frame_i->second.R * RIO).transpose() * (frame_j->second.T - frame_i->second.T) / 100;
            tmp_b.block<3, 1>(6, 0) = frame_j->second.pre_integration_wheel->delta_p - RIO.transpose() * frame_i->second.R.transpose() * frame_j->second.R * TIO + (frame_i->second.R * RIO).transpose() * frame_j->second.R * TIC[0] - RIO.transpose() * (TIC[0] - TIO);
...
bool LinearAlignmentWithWD(map<double, ImageFrame> &all_image_frame, Vector3d &g, VectorXd &x)
{
    int all_frame_count = all_image_frame.size();
    ...
    // ROS_STREAM(" result g     " << g.norm() << " " << g.transpose());
    if (fabs(g.norm() - G.norm()) > 0.5)
    {
        return false;
    }

    RefineGravityWithWD(all_image_frame, g, x);
    // s = (x.tail<1>())(0) / 100.0;
    //(x.tail<1>())(0) = s;
    ROS_DEBUG_STREAM(" refine     " << g.norm() << " " << g.transpose());
    return true;
