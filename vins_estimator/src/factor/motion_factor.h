...
class MotionFactor : public ceres::SizedCostFunction<2, 7, 7, 9,3>
{
  public:
    MotionFactor(){};
    virtual bool Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
    {
        ...
        double vx=parameters[3][0],vy=parameters[3][1],vz=parameters[3][2];
        ...
        //Eigen::Matrix<double, 2, 2> sqrt_info = Eigen::Vector3d(PITCH_N_INV, ROLL_N_INV).asDiagonal();
...
//                ROS_ASSERT(fabs(jacobian_pose_j.minCoeff()) < 1e8);
            ...
            if (jacobians[2])
            {
                Eigen::Map<Eigen::Matrix<double, 3, 9, Eigen::RowMajor>> jacobian_qpw(jacobians[2]);
