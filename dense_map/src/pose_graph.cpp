...
PoseGraph::PoseGraph()
{
    ...
    base_sequence = 1;
    use_imu = 0;
    octree = new pcl::octree::OctreePointCloudDensity<pcl::PointXYZRGB>(0.01);//org 0.01
...
void PoseGraph::addKeyFrame(KeyFrame *cur_kf, bool flag_detect_loop)
{
    ...
    if (!cur_kf->point_rgbd.empty())
    {
        ...
        // radius  https://zhuanlan.zhihu.com/p/176520634
        // test
        // 输入待滤波的原始点云指针
        ...
        pcl::toROSMsg(*cloud_filtered, tmp_pcl);
...
void PoseGraph::updatePath()
{
    ...
    for (auto &point_rgbd_vect : tmp_keyframelist)
    {
        ...
        for (auto &point_rgbd : point_rgbd_vect)
        {
            ...
            searchPoint.b = point_rgbd[5];
            ++update_count;
            if (octree->getVoxelDensityAtPoint(searchPoint) < 5) // org 5
            {
                octree->addPointToCloud(searchPoint, cloud);
                save_cloud->points.push_back(searchPoint);
            }
